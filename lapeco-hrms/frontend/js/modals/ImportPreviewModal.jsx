...
const ImportPreviewModal = ({ 
  ...
  // Sync editedData when importData changes
  useEffect(() => {
    if (importData) {
    ...
    <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
      <div className="modal-dialog modal-xl modal-dialog-scrollable">
        <div className="modal-content">
          <div className="modal-header text-white import-preview-header">
              ...
              <table className="table table-sm table-hover mb-0">
                <thead className="table-light sticky-top">
                  <tr>
                    ...
                    <th style={{ width: '150px' }}>Original Name</th>
                    <th style={{ width: '80px' }}>Original ID</th>
                ...
                <tbody>
                  {editedData.map((row, index) => (
                    ...
                    >
                      ...
                      <td>
                        <small className="text-muted">{row.idNumber}</small>
                      </td>
                      <td>
                        <div className="d-flex align-items-center gap-2">
                          {row.matchStatus === 'matched' && row.employeeName ? (
                            <strong className="text-success">{row.employeeName}</strong>
                          ) : (
                      ...
                      <td>
                        {row.matchStatus === 'matched' && row.employeeId ? (
                          <strong className="text-success">{row.employeeId}</strong>
                        ) : (
                          <span className="text-muted">-</span>
                        )}
                      </td>
